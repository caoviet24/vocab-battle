...
export type UploadType = "category" | "frame" | "user" | "word";
...
export function isStoredImagePath(segments: string[]): boolean {
  ...
  if (type === "frame") return segments.length === 2 && /\.webp$/.test(folder) && IMAGE_FILENAME.test(folder);
  if (type === "user") {
