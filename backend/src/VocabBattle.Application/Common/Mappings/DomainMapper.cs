...
{
    public static PlayerDto ToDto(Player player) => new(player.Id, player.Name, player.Score, player.FrameUrl);
    public static CategoryDto ToDto(Category category) =>
        new(category.Id, category.Name, category.Description, category.ImageUrl, category.CreatedAt);
    public static FrameDto ToDto(Frame frame) => new(frame.Id, frame.Name, frame.Url, frame.CreatedAt);
}
